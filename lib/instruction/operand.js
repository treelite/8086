...
parseHandlers.I = (cpu, inst, desc, modRM, offset) => {
    // 部分指令中操作数的字节数不遵守 opCodeByte.w
    // （比如：GRP1、RET等)
    // 所以直接使用 desc 中的描述
    // FIXME
    // 暂时不考虑 Io 的情况
    // 只支持 Ib、 Iv、Iw
    let w = desc.charAt(1) === 'b' ? 0 : 1;
    let value = cpu.read(cpu.ip + 1 + offset, w);
    return {
        descriptor: {
            writable: true,
            value: value
...
parseHandlers.J = (cpu, inst, desc) => {
    // opCodeByte.w 不生效
    let w = desc.charAt(1) === 'v' ? 1 : 0;
    let value = cpu.read(cpu.ip + 1, w);
    if (!w && value >> 7 === 1) {
        value |= 0xFF00;
    }
    return {
